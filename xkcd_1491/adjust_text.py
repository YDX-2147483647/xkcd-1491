...
def get_bboxes_pathcollection(sc, ax):
    ...
    trans_offset = sc.get_offset_transform()
    offsets = sc._offsets
    paths = sc.get_paths()
    ...
    if not trans_offset.is_affine:
        offsets = trans_offset.transform_non_affine(offsets)
        trans_offset = trans_offset.get_affine()
    ...
    if len(paths) and len(offsets):
        ...
        for p, o, t in zip(paths, offsets, transforms):
            result = get_path_collection_extents(
                transform.frozen(), [p], [t], [o], trans_offset.frozen()
            )
